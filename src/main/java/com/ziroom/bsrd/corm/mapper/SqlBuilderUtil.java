package com.ziroom.bsrd.corm.mapper;

import com.google.common.base.CaseFormat;
import com.ziroom.bsrd.basic.annotation.Column;
import com.ziroom.bsrd.basic.annotation.Table;
...
import com.ziroom.bsrd.corm.annotation.CormIgnore;
...
public class SqlBuilderUtil {
    ...
    private static String whereOr(Condition condition, int i) {
    ...
    private static CormIgnore findIgnore(Class<?> clazz, Method getter, String propertyName) {
        CormIgnore ignore = getter.getAnnotation(CormIgnore.class);
        if (ignore == null) {
            java.lang.reflect.Field field = findField(clazz, propertyName);
            if (field != null) {
                ignore = field.getAnnotation(CormIgnore.class);
            }
        }
        return ignore;
    }
    ...
    private static java.lang.reflect.Field findField(Class<?> clazz, String propertyName) {
        if (clazz.equals(Object.class)) {
            logger.warn("class[{}] not find field[{}], please check your params!", clazz.getName(), propertyName);
            return null;
        }
        try {
            return clazz.getDeclaredField(propertyName);
        } catch (NoSuchFieldException e) {
//            logger.warn("class[{}] no field[{}]", clazz.getName(), propertyName);
        }
    ...
    private static Map<String, PropertyDescriptor> findProperties(Class<?> clazz) {
        if (clazz == null) {
            throw new IllegalArgumentException(
                    "Please check your params, can not find entity class. Check Condition, Field, OrderBy class."
            );
        }
        ...
        try {
            ...
            for (PropertyDescriptor property : propertyDescriptors) {
                ...
                Method getter = property.getReadMethod();
                CormIgnore ignore = findIgnore(clazz, getter, property.getName());
                if (ignore != null) {
                    continue;
                }
